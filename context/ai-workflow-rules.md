...
- Work **spec-driven**: read `context/specs/NN-*.md` for the active unit and implement exactly that scope.
...
- Implement only the active unit. Do not start the next UH unit early.
...
- If a unit mixes schema + large UI + PWA in one session and verification fails, stop and ask to split — do not silently expand scope.
- Prefer: schema/lib first, then staff UI, then client UI across ordered units (already reflected in UH01–UH07).
...
- Do not invent OTA, CashSession hard gates, or staff-only F&B as the primary order path.
