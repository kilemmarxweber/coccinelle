...
export default async function AgencePassagesPage({ params }: PageProps) {
  const { organizationId, branchId } = await params;
  const branch = await requireBranchContext({
    organizationId,
    branchId,
    requireModule: "agence",
    requireDashCard: "embarquement",
  });
  return (
    <BranchModulePlaceholder
      ...
      title="Embarquement"
      description="Scan QR et passages pour les départs de la branche."
    />
  );
}
