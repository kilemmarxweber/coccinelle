    ...
    <div className="mx-auto flex w-full min-w-0 max-w-2xl flex-col gap-6 px-[max(1rem,env(safe-area-inset-left))] py-5 pr-[max(1rem,env(safe-area-inset-right))] pb-8 md:max-w-4xl md:px-6">
      <div className="space-y-1.5">
        <p className="text-pretty text-sm leading-relaxed text-muted-foreground">
          Création du compte (email + mot de passe généré côté serveur), ajout immédiat à l’organisation, et envoi
          d’un email de confirmation avec le mot de passe temporaire (configurez <code className="rounded bg-muted px-1 text-xs">RESEND_API_KEY</code> pour l’envoi réel).
        </p>
      </div>
