...
It should give contributors and AI agents one consistent set of
rules. Include these sections in plain Markdown:

- General — cross-cutting principles (small modules, fix root
  causes, do not mix unrelated concerns in one component or route)
- TypeScript — strictness, typing rules, how to handle unknown or
  external input at boundaries
- [Your framework — e.g. Next.js] — server vs client boundaries,
  routing and data-fetching patterns, where business logic lives
- Styling — tokens, no hardcoded values, links to design context
  if we use ui-context.md or similar
- API Routes — validate input first, auth and ownership before
  mutations, consistent response shapes
- Data and Storage — what belongs in the database vs file or blob
  storage vs cache
- File Organization — each top-level folder and what belongs there
  (use our real or planned paths)

My stack and preferences: [paste framework, styling system, API
style, and non-negotiables from planning]
Write imperative rules (must / do not), not vague suggestions.
Where one rule depends on another, say so briefly.
```
...
*Good:* `"Create sign-in and sign-up pages using Clerk   components with a two-panel layout on desktop and   form-only on mobile. Use proxy.ts for route protection,   not middleware.ts."`
...
*Built with the Six-File Context System.*
